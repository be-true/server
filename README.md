...
- Конфигурация через envs с использованием библиотеки  @be-true/config
    - Добавить указание имени для поля
    - Перезапись значений через JSON
    - Не выбрасывать ошибку при обращении к переменной
    - Добавить метод валидации у класса, который возвращает ошибки для item-ов
    - Возможно перевести с get-ов на инициализацию в конструкторе
